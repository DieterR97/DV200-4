...
function EditProfileModal() {
    ...
    const handleSave = async () => {
        try {
            // Create a new object with updated user data, including the password field
    ...
    const handleConfirmDelete = async () => {
        try {
            ...
            const response = await axios.post(`http://localhost:5000/api/auth`, { email: parsedUserData.email, password }, config);
            // const response = await axios.post(`http://localhost:5000/api/auth`, parsedUserData, config);

            if (response.status === 200) {
                // Password is correct; proceed with profile deletion
                await axios.delete(`http://localhost:5000/api/user/${parsedUserData._id}`);
    ...
    return (
        <>
            <Button className={styles.btnEditProfile} onClick={handleShow}>
                Edit Profile
            </Button>
