...
const Comments = ({ postID }) => {
    ...
    //             // Process the data and set it in your component's state
    //             setComments(data);
    //         } else {
    ...
    const handleThumbsUpClick = async (commentID, index) => {
        try {
            ...
            if (storedUserData) {
                ...
                const response = await axios.post(`http://localhost:5000/api/comment/upvote/${commentID}`, parsedUserData, config);
    ...
    const handleThumbsDownClick = async (commentID, index) => {
        try {
            ...
            if (storedUserData) {
                ...
                // Use `config` in your Axios request
                const response = await axios.post(`http://localhost:5000/api/comment/downvote/${commentID}`, parsedUserData, config);
    ...
    const handleFlagClick = async (commentID, index) => {
        try {
            ...
            const response = await axios.put(`http://localhost:5000/api/comment/flag/${commentID}`, null, config);
    ...
    //                 <span onClick={handleThumbsUpClick(comment._id)}>
    ...
    //                 <span onClick={handleThumbsDownClick(comment._id)}>
    ...
    //                         </span>
    //                         <span className={styles.postedTime}> {formatDate(comment.date)}</span>
    //                     </div>

    //                     <span className={styles.flagIcon} onClick={handleFlagClick(comment._id)}>
    //                         {commentIsFlagged &&
                        ...
                        .map(({ comment, index }) => (
                            ...
                            <div key={comment._id} className={styles.comments}>
                                ...
                                <div className={styles.voteicons}>
                                    ...
                                    <span onClick={() => handleThumbsUpClick(comment._id, index)}>
                                    ...
                                    <span className={styles.voteCountContainer}>{voteCount[index]}</span>

                                    <span onClick={() => handleThumbsDownClick(comment._id, index)}>
                                ...
                                <div className={styles.commentscontainer}>
                                    <div className={styles.userheading}>
                                        ...
                                        <span className={styles.flagIcon} onClick={() => handleFlagClick(comment._id, index)}>
